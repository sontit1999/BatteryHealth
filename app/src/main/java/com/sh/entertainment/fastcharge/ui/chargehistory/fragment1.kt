...
class fragment1 : Fragment() {
    ...
    fun initHistoryChart() {
        ...
        for (i3 in 0..23) {
            val level: Int =
                HistoryPref.getLevel(requireContext(), HistoryPref.getKeyFromTime(i, i3))
            if (i3 != 0) {
                if (level != -1) {
                    if (i3 % 2 == 0) {
                        arrayList.add(
        ...
        instance2.add(5, -1)
        val level2: Int =
            HistoryPref.getLevel(requireContext(), HistoryPref.getKeyFromTime(instance2[5], 0))
        if (level2 != -1) {
    ...
    fun removeDay() {
        val i = Calendar.getInstance()[5]
        for (i2 in 0..23) {
            HistoryPref.removeLevel(requireContext(), HistoryPref.getKeyFromTime(i - 3, i2))
        }
    }

    companion object {
        fun newInstance(i: Int, str: String?): fragment1 {
