...
import com.procharger.fastprocharrging.quickcharge.common.extension.contentView
...
abstract class BaseActivity<V : BaseView, P : BasePresenterImp<V>> : AppCompatActivity(), BaseView {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        registerReceiver(broadcastReceiverExitApp, IntentFilter(com.procharger.fastprocharrging.quickcharge.common.Constants.ACTION_EXIT_APP))
    }
...
//        window.setFlags(WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS, WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS);
    ...
    inner class BroadcastReceiverExitApp : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            if (intent?.action?.equals(com.procharger.fastprocharrging.quickcharge.common.Constants.ACTION_EXIT_APP) == true) {
                self.finish()
            }
        }
    }
